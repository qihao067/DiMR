...
Reference:
    https://github.com/baofff/U-ViT/blob/main/train.py
"""
...
def LSimple(x0, nnet, schedule, loss_coeffs, **kwargs):
    ...
    target = multi_scale_targets(eps, levels = len(prediction), scale_correction = True)
    loss = 0
...
def train(config):
    ...
    def train_step(_batch):
        ...
        if config.train.mode == 'cond':
            ...
            loss = LSimple(_z, nnet, _schedule, loss_coeffs=config.loss_coeffs, y=_batch[1])
        else:
    ...
    def dpm_solver_sample(_n_samples, _sample_steps, **kwargs):
        ...
        def model_fn(x, t_continuous):
            ...
            eps_pre = nnet_ema(x, t, log_snr, **kwargs)[-1]
            return eps_pre
