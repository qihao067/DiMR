"""This file contains code for datasets registration.

This file may have been modified by Bytedance Ltd. and/or its affiliates (“Bytedance's Modifications”).
All Bytedance's Modifications are Copyright (year) Bytedance Ltd. and/or its affiliates. 

Reference:
    https://github.com/baofff/U-ViT/blob/main/datasets.py
"""
...
class ImageNet256Features(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder
    def __init__(self, path, cfg=False, p_uncond=None):
        ...
        self.categorys = torch.arange(0, 1000).repeat_interleave(50)
    ...
    def sample_label(self, n_samples, category=-1, accelerator=None, device=None):
        if category==-1:
            class_lab = torch.randint(0, 1000, (n_samples,), device=device)
            with open("class_lab.txt", "a") as file:
                file.write(str(class_lab.cpu().numpy()) + '\n')
            return class_lab
        else:
            class_lab = self.categorys[(category*8 + accelerator.process_index) * n_samples : (category*8 + accelerator.process_index+1) * n_samples].to(device)
            with open("class_lab.txt", "a") as file:
                file.write(str(class_lab.cpu().numpy()) + '\n')
            return class_lab
...
class ImageNet(DatasetFactory):
    ...
    def sample_label(self, n_samples, category=-1, accelerator=None, device=None):
        if category==-1:
            class_lab = torch.multinomial(self.cnt, n_samples, replacement=True).to(device)
            with open("class_lab.txt", "a") as file:
                file.write(str(class_lab.cpu().numpy()) + '\n')
            return class_lab
        else:
            class_lab = self.categorys[(category*accelerator.num_processes + accelerator.process_index) * n_samples : (category*accelerator.num_processes + accelerator.process_index+1) * n_samples].to(device)
            with open("class_lab.txt", "a") as file:
                file.write(str(class_lab.cpu().numpy()) + '\n')
            return class_lab
